...
class SmallCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.pool(self.act(self.norm1(self.conv1(x))))
        x = self.pool(self.act(self.norm2(self.conv2(x))))
        x = x.view(x.size(0), -1)
        x = self.dropout(self.act(self.fc_norm(self.fc1(x))))
        return self.fc2(x)
...
def parse_args() -> TrainConfig:
    ...
    for f_name in ["optimizer", "scheduler", "activation", "norm", "data_dir"]:
