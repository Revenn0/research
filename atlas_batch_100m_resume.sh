...
# Ordem priorizada: blend+LS (campeão) → baseline → demais pendentes
set -e
cd "$(dirname "$0")"
...
echo "========== RETOMADA BATCH 100M (blend+LS → baseline → resto) =========="

# Concluídos anteriormente (skip automático se já no log)
...
echo "=== PRIORIDADE 1: exp-100m-010 | exp-027 blend+LS ex-campeão ==="
...
echo "=== PRIORIDADE 2: exp-100m-baseline | referência @100M ==="
...
echo "=== RESTO: experimentos pendentes ==="
